...
if exists build (rmdir /S /Q build || goto :exit)
mkdir build || goto :exit
cd build || goto :exit
copy ..\cmake\config.cmake . || goto :exit
...
cmake -DCMAKE_INSTALL_PREFIX=%LOCALAPPDATA%\Programs\TVM .. || goto :exit
REM Needed because otherwise LINK.EXE cannot find tvm.lib
set "LINK=/LIBPATH:%CD%\Debug" || goto :exit
REM TODO: Why are .pdb files not copied to the install direcotry?
cmake --build . --target install --parallel 24 || goto :exit
