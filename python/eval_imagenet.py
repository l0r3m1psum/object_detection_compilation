...
# ================= Configuration =================
ONNX_MODEL_PATH = "build/resnet18-v2-7.onnx"
ONNX_MODEL_PATH_QUANT = "build/resnet18wd4_pt_224_qdq_int8.onnx"
...
def main():
    ...
    print(f"Loading ONNX model from {ONNX_MODEL_PATH_QUANT}...")
    providers = ['CUDAExecutionProvider', 'CPUExecutionProvider']
    session_quant = ort.InferenceSession(ONNX_MODEL_PATH_QUANT, providers=providers)
    session = ort.InferenceSession(ONNX_MODEL_PATH, providers=providers)
    
    # Get the name of the model's input layer
    input_name = session_quant.get_inputs()[0].name

    correct_tvm = 0
    correct_quant_top1 = 0
    correct_top1 = 0
    total_samples = 0
    ...
    for images, labels in val_loader:
        ...
        outputs = session.run(None, {"data": images_np})[0]
        predictions = np.argmax(outputs, axis=1)

        outputs_quant = session_quant.run(None, {input_name: images_np})[0]
        predictions_quant = np.argmax(outputs_quant, axis=1)
        ...
        correct_quant_top1 += np.sum(predictions_quant == labels_np)
    ...
    top1_accuracy_quant = correct_quant_top1 / total_samples
    top1_accuracy_tvm = correct_tvm / total_samples
    print("\n" + "="*40)
    print(f"Total Images Evaluated : {total_samples}")
    print(f"Top-1 Accuracy         : {top1_accuracy * 100:.2f}%")
    print(f"Top-1 Accuracy (QUANT) : {top1_accuracy_quant * 100:.2f}%")
