...
class FuncInjectRewriter(relax.PyExprMutator):
	...
	def visit_function_(self, func: relax.Function) -> relax.Expr:
		...
		with self.builder_.function("dsadsdsa", func.params, dict(func.attrs)):
			...
			self.builder_.emit_func_output(gv)
		res = self.builder_.get()["dsadsdsa"]
		# FIXME!!!
		res.show()
		return res
...
class InjectFunctionTransform:
	...
	def transform_module(self, mod: ir.IRModule, ctx: ir.transform.PassContext) -> ir.IRModule:
		...
		for gv in mod.get_global_vars():
			if gv.name_hint == self.prep_func_name:
				prep_gv = gv
				break
		else:
			raise ValueError("'%s' not found in module." % self.prep_func_name)
