...
def FixSelectCondition(func, mod, ctx):
    """
    Simplify Select conditions
    Select(Const(0, Type) < Cast(Type, BoolExpr), ...) -> Select(BoolExpr, ...)
    (Usually generated by schedulings with compute_inline)
    """

    def post_order(op):
        if not isinstance(op, tir.Select):
            return None

        cond = op.condition
        if isinstance(cond, tir.LT):
            if isinstance(cond.a, tir.IntImm) and cond.a.value == 0:
                if isinstance(cond.b, tir.Cast):
                    inner_bool_expr = cond.b.value
                    return tir.Select(inner_bool_expr, op.true_value, op.false_value)

        return None

    pre_order = None
    new_body = tir.stmt_functor.ir_transform(func.body, pre_order, post_order,
        ('tir.Select',)
    )
